@echo off
echo.
echo BUILDING LIBRARY "As3corelib Library"...

REM 0. Do a preflight check and see if the project needs to be built
SETLOCAL
FOR /F "tokens=* USEBACKQ" %%g IN (`d:\_DEV_\_BUILD_\NODE\win\node.exe d:\_DEV_\_GIT_\claudius-iacob-eu\SIRIUS\sirius.js d:\_DEV_\_GIT_\github\as3corelib-library --dirty-check-only`) do (SET "PROJECT_STATE=%%g")
...
d:\_DEV_\_BUILD_\NODE\win\node.exe d:\_DEV_\_GIT_\claudius-iacob-eu\SIRIUS\sirius.js d:\_DEV_\_GIT_\github\as3corelib-library --cfg-only --silent

:deps
REM 2. Recursively update and build dependencies if any:
echo Updating and building dependencies of "As3corelib Library"...
...
%javaDir%\java -jar %sdkJarsDir%\compc.jar +flexlib %frameworksDir% -load-config d:\_DEV_\_GIT_\github\as3corelib-library\.build-scripts\lib-config.xml
if errorlevel 1 goto buildfailed
if not errorlevel 1 goto cleanexit

:buildfailed
echo BUILDING LIBRARY "As3corelib Library" FAILED, EXITING.
exit /b %ERRORLEVEL%

:cleanexit
echo BUILDING LIBRARY "As3corelib Library" COMPLETE, EXITING.
exit /b %ERRORLEVEL%

:noactionexit
echo "As3corelib Library" ALREADY BUILT, EXITING.
exit /b %ERRORLEVEL%